...
* 多态性有助于提高程序的可扩展性。
		...
		1.可以把控制模块与被操作的对象分开。
		2.可以添加已定义的新对象，并能管理该对象。
		3.可以添加新类（已有类的派生类）的新对象，并能管理该对象。
		（代码适应未来，只添加不用删除）		
		
		4.简单工厂模块（35cpp-01.cpp）
...
* 构造函数不得是虚函数。
* 如果要操作具有继承关系的类的动态对象，最好使用析构函数。特别是在析构函数需要完成一些有意义的操作－－比如释放内存。
* 析构函数还可以是纯虚的。

		1.对于一个没有任何接口的类，如果想要将它定义成抽象类，只能将虚析构函数声明为纯虚的；
		2.通常情况下在基类中纯虚函数不需要实现；
		3.例外是纯虚析构函数要给出实现。（给出一个空的实现即可）
		（34cpp-2.cpp）
...
#### 1.反射
	...
	动态创建对象
	->对原有的类不做任何改，只需要增加一个宏就能够实现动态创建
	－>(动态创建的优势）
	避免了if else 的使用，增加了可扩展性。
